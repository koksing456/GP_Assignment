...
float TxProjection = 0, TyProjection = 0;
...
float xxh = 0.0, yyh = 0.0, zzh = 0.0; //hand
...
LRESULT WINAPI WindowProcedure(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYDOWN:
		...
		else if (wParam == VK_SHIFT) { //left hand finger down
			if (RxleftFinger > 0.5) {
				RxleftFinger -= rSpeed1,
					xxh = 1.0, yyh = 0.0, zzh = 0;
			}
		}
		...
		else if (wParam == VK_LEFT) { //shoot gun
			popOut = true;
			isShootY = true;
			isShoot = true;
			lx = 0.5, ly = 0.0, lz = 0.0;
			lx2 = 0.0, ly2 = 0.5, lz2 = 0.0;
		}
...
void projection()
{
	...
	//glRotatef(RaX, Rx, Ry, Rz); // rotate view in x axis
	//glRotatef(RaY, Rx, Ry, Rz); // rotate view in y axis
	//glRotatef(RaZ, Rx, Ry, Rz); // rotate view in z axis
...
void drawLeftArm() //** //changes <<<<<
{
	...
	glRotatef(RxleftHand, xxh, yyh, zzh);
	...
	//finger movement
	glTranslatef(0.0, -0.9, 0.0);
	glRotatef(RxleftFinger, xxh, yyh, zzh);
	glTranslatef(0.0, 0.9, 0.0);
	//finger movement
	//finger 1
	glPushMatrix();
...
void drawRightArm() //** //changes <<<<<
{
	...
	//hand movement
	glTranslatef(0.0, -0.8, 0.0);
	glRotatef(RxrightHand, xxh, yyh, zzh);
	...
	//finger movement
	glTranslatef(0.0, -0.9, 0.0);
	glRotatef(RxrightFinger, xxh, yyh, zzh); /////////////////////
	glTranslatef(0.0, 0.9, 0.0);
	//finger movement
...
void drawGun() {
	...
	glColor3f(1.0, 1.0, 0.0);
	glPushMatrix();
	glTranslatef(-0.6, -0.26, 0.0);
	glScalef(1.5, 3.2, 1.0);
	kelvinDrawCube(size);
	glColor3f(0.0, 0.0, 0.0);
	glPushMatrix();
	glTranslatef(0.0, 0.0, -0.01);
	glScalef(0.0, 1.0, 1.1);
	kelvinDrawCubeInLine(size);
	glPopMatrix();
	glPopMatrix();
...
void drawMiddleLaser() {
	float radius = 0.13, slideNo = 30, stackNo = 30;
	...
	kelvinDrawSphere(radius, slideNo, stackNo);
...
void drawLeftGun() {
	...
	//LASER
	glPushMatrix();
	glTranslatef(-0.36, 0.08, 0.1);
	glScalef(0.5, 0.5, 0.5);
	drawMiddleLaser();
	glPopMatrix();

	glPushMatrix();
	glTranslatef(-0.35, 0.2, 0.1);
	glScalef(0.5, 0.5, 0.5);
	drawUpperAndLowerLaser();
	glPopMatrix();
